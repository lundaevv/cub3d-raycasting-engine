...
void check_door_in_range(t_game *game_dt) {
  ...
  const t_vec_int door_pos = (t_vec_int){
      (game_dt->player.pos.x + cos_val * floor(TILE_SIZE * 1.1)) / TILE_SIZE,
      (game_dt->player.pos.y + sin_val * floor(TILE_SIZE * 1.1)) / TILE_SIZE};
