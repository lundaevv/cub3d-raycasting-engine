...
static void draw_wall_column(t_game game_dt, t_raycast_data ray_data,
  ...
  while (draw_start <= draw_end) {
    putp(&game_dt.mlx.frame, col_x, draw_start,
         getp(tex, ray_data.tex_x * tex->w, tex_y));
    tex_y += tex->h / wall_h;
    draw_start++;
  }
}
...
unsigned int getp_door_sprite(t_game *game_dt, double tex_x, int y, int frame) {
...
static void draw_door(t_game *game_dt, t_raycast_data ray_data, double wall_h,
                      const int col_x) {
  ...
  while (draw_start <= draw_end) {
    putp(&game_dt->mlx.frame, col_x, draw_start,
         getp_door_sprite(game_dt, ray_data.tex_x, tex_y, frame));
